...
const { findFolderupload, uploadFileToDrivett, uploadFileToDrive,createDriveFolder,findNestedFolder,deleteTempFiles ,findFolder} = require("./utils/googleDrive");
...
app.post("/api/noteClickCount", async (req, res) => {
  ...
  try{
    await connectionUserdb.query('UPDATE notes SET totalClicks = totalClicks + 1 WHERE id = ?', [id]);
...
app.get("/api/adminPage", async (req, res) => {
  ...
  try {
    ...
    const [results] = await connectionUserdb.query(query, [ req.userid.userId]);
