...
exports.createPaymentOrder = async (req, res) => {
  const { amount, customerEmail, customerPhone, orderType, credits } = req.body;
  ...
  try {
    ...
    // Fallback to stored gmail if email not provided
    if (!email) {
      const [userRows] = await connectionUserdb.query("SELECT gmail FROM users WHERE id = $1", [userId]);
      if (userRows.length > 0 && userRows[0].gmail) email = userRows[0].gmail;
    }
    ...
    // Build order ID based on type: SV_ for support, CR_userId_credits_ for credits
    const suffix = `${Date.now()}_${Math.random().toString(36).substring(2, 6)}`;
    const orderId = (orderType === 'credits' && credits)
      ? `CR_${userId}_${credits}_${suffix}`
      : `SV_${userId}_${suffix}`;
...
exports.getTransactionHistory = asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const sql =
    "SELECT * FROM user_transactions WHERE user_id = $1 ORDER BY timestamp DESC";
  const [rows] = await connectionUserdb.query(sql, [userId]);
  res.status(200).json({ success: true, data: rows });
});

/*
 * ─── UTR MANUAL VERIFICATION (commented out - using Cashfree gateway instead) ───
 *
exports.submitUpiUtr = asyncHandler(async (req, res) => {
  ...
  if (existing.length > 0) {
    return res.status(409).json({
      error: "This UTR has already been submitted. Contact support if you need help.",
    });
  }
